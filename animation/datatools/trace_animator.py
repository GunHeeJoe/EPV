...
class TraceAnimator:
    ...
    def animate_ball(
        ...
        #print(highlight_time , " : ",t)
        if t in highlight_time:
            #print(highlight_frame , " : ",t)
            scat.set_color("yellow")  # 또는 특정 시간 범위에 맞는 다른 색상
            scat.set_edgecolor("k")
            plot.set(color="yellow", linewidth=6)
            
            #이벤트를 수행하는 시작 t
            t_from = highlight_time[0]
            plot.set_data(x[t_from : t + 1], y[t_from : t + 1])
        else:
            scat.set_color("w")  # 기본 색상으로 되돌림
            scat.set_edgecolor("k")
            plot.set_color('black')
            plot.set(color="black",linewidth=1)
    ...
    def run(self):
        ...
        # t : 시간단위
        # time, frame정보랑 헷갈렸는데, t를 출력해보면 0,1,2,3,4방식으로 출력됨 -> 즉 해당구간의 tracking-data를 의미함
        def animate(t):
            for key in self.trace_dict.keys():
                ...
                #실제 패스 이벤트 장면만 highlight
                if ball_args is not None:
                    TraceAnimator.animate_ball(t, *ball_args, self.highlight_time)
