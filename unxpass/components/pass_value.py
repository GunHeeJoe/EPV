...
from unxpass.channels import nb_opp_in_path, get_pass_success_probability, get_pressure_line
...
class PassValueComponent():
    """The pass value probability component.

    From any given game situation where a player controls the ball, the model
    estimates the value probability of a pass attempted towards a potential
    ...
    def __init__(self, model):
        ...
        logger = TensorBoardLogger("./runs/", name='pass_value')
    ...
    def _get_metrics(self, y, y_hat):
        ...
        #Pass Value Model은 [-1, 1]사이의 값이므로 확률값이 아님    
        # return {
        #     "brier": brier_score_loss(y, y_hat),
        #     "log_loss": log_loss(y, y_hat),
        #     "roc_auc": roc_auc_score(y, y_hat),
        #     # "ECE" : self.expected_calibration_error(y_hat, np.array(_y)),
        #     "soccermap_ece" : soccermap_ece,
        #     "epv_ece" : epv_ece,
        #     "bce_l1" : bce_l1(tensor_y_hat, tensor_y),
        # }
    
    #pass value에서는 여기서 test진행
    #base코드에서가 아닌 상속받은 해당 부분에서 진행
    def test(self, test_dataset, batch_size=1, num_workers=0, pin_memory=True, config={}) -> Dict:
        ...
        for batch in track(dataloader,description="pass value model test loading"):
            loss, y_hat, y = self.model.step(batch)
            all_preds.append(y_hat)
            all_targets.append(y)

        #Pass-Value model에서는 테스트할때 기존 학습시 사용한 score_xg과는 다르게
        #goal/no-goal인 scores정보를 활용하여 학습함
...
class PytorchSoccerMapModel(pl.LightningModule):
    ...
    def __init__(
        ...
        # it also allows to access params w 'self.hparams' attribute
        self.save_hyperparameters()

        self.model = SoccerMap(in_channels=16)
    ...
    def forward(self, x: torch.Tensor):
        ...
        #sigmoid로 부터 나온 [0,1]의 확률 값을 [-1, 1]로 선형변환해줌
...
class ToSoccerMapTensor:
    ...
    def __call__(self, sample):
        ...
        #value model은 도착한 패스의 pixel부분의 value_label활용함
        target = int(sample["value_label"])
        ...
        # Output
        matrix = np.zeros((16, self.y_bins, self.x_bins))
        ...
        # CH14 : Pass Success Probability
        # Pretrained_model : 같은 데이터셋으로 학습한 패스성공확률모델 활용함
        matrix[13, :, :] = get_pass_success_probability(matrix)

        # CH 15 : OutPlayed Numbers
        # 수비수들이 존재하는 셀의 Polygon에 passline이 intersect하는지 파악하는 로직
        # x_bin_def, y_bin_def는 bottom-left의 셀의 좌표가 설정되어있으므로 해당 셀 전체의 Polygon를 설정해줌
        opponent_cells = [Polygon([(x, y), (x+1, y), (x, y+1), (x+1, y+1)]) for x, y in zip(x_bin_def, y_bin_def)]
        for i in range(self.y_bins):
            for j in range(self.x_bins):
                start_coo = (x_ball+0.5, y_ball+0.5)
                end_coo = (xx[0][j], yy[i][0])
                passline = LineString([start_coo, end_coo]).buffer(0.5)

                # 교차하는 셀 수 계산
                outplayed_numbers= sum(passline.intersects(cell) for cell in opponent_cells)
                matrix[14, i, j] = outplayed_numbers

        # CH 16 : Pressure line index
        # 수비라인의 x좌표를 바탕으로 클러스터링했을 때, 각 셀에서 가장 가까운 압박라인의 인덱스
        matrix[15, :, :] = get_pressure_line(xx, x_bin_def, self.y_bins)
